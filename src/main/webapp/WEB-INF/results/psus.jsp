...
<html>
    ...
    <body id="top">
        ...
        <form class="add-button-container">
            <button type="submit" formmethod="get" formaction="addPsu"><i class="fa fa-plus-square-o" aria-hidden="true"></i>Add <%=fileName.substring(0, fileName.length()-1)%></button>
        </form>

        <%if (psus == null || psus.isEmpty()){%>
        <div class="no-items-container">
            <p>No <%=fileName%> found in the database or no components with selected filter found. Add some.</p>
            <form class="filter-reset-button-container" action="psus" method="get">
        ...
        <div class="filters-container">
            ...
            <div class="main-filters-container">
                ...
                <!-- Campo ID -->
                <form action="psus" method="get" class="filter-item">
                ...
                <!-- Select per efficiency -->
                <form action="psus" method="get" class="filter-item">
                    <label for="efficiency">Efficiency:</label>
                    <select id="efficiency" name="efficiency">
                        <option value="">All</option>
                    ...
                    <button type="submit">Filter by efficiency</button>
                </form>

                <form action="psus" method="get" class="filter-item">
                    <label for="minWattage">Min Wattage:</label>
                    <input type="number" id="minWattage" name="minWattage" step="1" required>
                    <button type="submit">Filter by Wattage</button>
                </form>

                <form action="psus" method="get" class="filter-item">
                    <label for="maxLenght">Max Length:</label>
                    <input type="number" id="maxLenght" name="maxLenght" step="1" required>
                    <button type="submit">Filter by Length</button>
                </form>
            </div>
            ...
            <div class="composite-filter-container">
                ...
                <form action="psus" method="get">
                    ...
                    <!-- Select per compositeType -->
                    <label for="compositeType">Type:</label>
                    <select id="compositeType" name="compositeType">
                        <option value="">All</option>
                        <% for (String type : types) { %>
                        <option value="<%= type %>"><%= type %></option>
                        <% } %>
                    ...
                    <!-- Select per compositeEfficiency -->
                    <label for="compositeEfficiency">Efficiency:</label>
                    <select id="compositeEfficiency" name="compositeEfficiency">
                        <option value="">All</option>
                        <% for (String efficiency : efficiencies) { %>
                        <option value="<%= efficiency %>"><%= efficiency %></option>
                        <% } %>
                    ...
                    <!-- compositeMinWattage tra X e Y -->
                    <label for="compositeMinWattage">Min Wattage:</label>
                    <input type="number" id="compositeMinWattage" name="compositeMinWattage" step="1">
                    <label for="compositeMaxWattage">Max Wattage:</label>
                    <input type="number" id="compositeMaxWattage" name="compositeMaxWattage" step="1">

                    <!-- compositeMinLenght tra X e Y -->
                    <label for="compositeMinLenght">Min Length:</label>
                    <input type="number" id="compositeMinLenght" name="compositeMinLenght" step="1">
                    <label for="compositeMaxLenght">Max Length:</label>
                    <input type="number" id="compositeMaxLenght" name="compositeMaxLenght" step="1">
        ...
        <div class="list-container">
            <%for (PSU psu : psus){%>
            <div class="component">
                <img src="<%=psu.getImage_URL()%>" alt="<%=psu.getName()%>">

                <div class="component-info-container">
                    <%
                        String componentName = psu.getName();
                    ...
                    <div class="group-container">
                        ...
                        <p class="group-content"><%=psu.getId()%></p>
                    </div>
                    <div class="group-container rating-container">
                        <p class="group-label">Rating</p>
                        <p class="group-content"><i class="fa fa-star" aria-hidden="true"></i><%=psu.getRating()%></p>
                    </div>
                    <div class="group-container">
                        <p class="group-label">Price</p>
                        <p class="group-content">€<%=psu.getPrice()%></p>
                    </div>
                    <div class="group-container">
                        <p class="group-label">Shop URL</p>
                        <a href="<%=psu.getShop_URL()%>"><p class="group-content">Link</p></a>
                    </div>
                    <div class="group-container">
                        <p class="group-label">Type</p>
                        <p class="group-content"><%=psu.getType()%></p>
                    </div>
                    <div class="group-container">
                        <p class="group-label">Efficiency</p>
                        <p class="group-content"><%=psu.getEfficiency()%></p>
                    </div>
                    <div class="group-container">
                        <p class="group-label">Wattage</p>
                        <p class="group-content"><%=psu.getWattage()%>W</p>
                    </div>
                    <div class="group-container">
                        <p class="group-label">Length</p>
                        <p class="group-content"><%=psu.getLenght()%>mm</p>
                    </div>
                </div>

                <form class="component-buttons-container">
                    <input type="hidden" name="id" value="<%=psu.getId()%>">
                    <button type="submit" formmethod="get" formaction="editPsu"><i class="fa fa-pencil" aria-hidden="true"></i>Edit</button>
                    <button type="button" class="remove-button" data-id="<%=psu.getId()%>" data-filename="<%=fileName%>"><i class="fa fa-times-circle-o" aria-hidden="true"></i>Remove</button>
                </form>
            </div>
            <%}%>
        </div>
        ...
        <div id="remove-popup" class="popup">
            <div class="popup-content">
                <h2></h2>
                <form id="remove-form" action="removePsu" method="post">
