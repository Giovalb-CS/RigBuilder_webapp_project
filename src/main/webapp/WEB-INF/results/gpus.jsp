...
<html>
    ...
    <body id="top">
        ...
        <form class="add-button-container">
            <button type="submit" formmethod="get" formaction="addGpu"><i class="fa fa-plus-square-o" aria-hidden="true"></i>Add <%=fileName.substring(0, fileName.length()-1)%></button>
        </form>

        <%if (gpus == null || gpus.isEmpty()){%>
            <div class="no-items-container">
                <p>No <%=fileName%> found in the database or no components with selected filter found. Add some.</p>
                <form class="filter-reset-button-container" action="gpus" method="get">
        ...
        <%} else {%>
            ...
            <div class="filters-container">
                ...
                <div class="main-filters-container">
                    ...
                    <!-- Campo ID -->
                    <form action="gpus" method="get" class="filter-item">
                    ...
                    <form action="gpus" method="get" class="filter-item">
                        ...
                        <button type="submit">Filter by Memory Type</button>
                    </form>
                </div>
                ...
                <div class="composite-filter-container">
                    ...
                    <form action="gpus" method="get">
                        ...
                        <!-- Select per Memory Type -->
                        <label for="compositeMemoryType">Memory Type:</label>
                        <select id="compositeMemoryType" name="compositeMemoryType">
                            <option value="">All</option>
                            <% for (String memoryType : memoryTypes) { %>
                                <option value="<%= memoryType %>"><%= memoryType %></option>
                            <% } %>
                        ...
                        <!-- Select per Min Slot width -->
                        <label for="compositeMaxSlotWidth">Max Slot width:</label>
                        <input type="number" id="compositeMaxSlotWidth" name="compositeMaxSlotWidth" step="0.01">

                        <!-- Select per Max lenght -->
                        <label for="compositeMaxLenght">Max lenght:</label>
                        <input type="number" id="compositeMaxLenght" name="compositeMaxLenght" step="0.01">
            ...
            <div class="list-container">
                <%for (GPU gpu : gpus){%>
                    <div class="component">
                        <img src="<%=gpu.getImage_URL()%>" alt="<%=gpu.getName()%>">

                        <div class="component-info-container">
                            <%
                                String componentName = gpu.getName();
                            ...
                            <div class="group-container">
                                ...
                                <p class="group-content"><%=gpu.getId()%></p>
                            </div>
                            <div class="group-container rating-container">
                                <p class="group-label">Rating</p>
                                <p class="group-content"><i class="fa fa-star" aria-hidden="true"></i><%=gpu.getRating()%></p>
                            </div>
                            <div class="group-container">
                                <p class="group-label">Price</p>
                                <p class="group-content">€<%=gpu.getPrice()%></p>
                            </div>
                            <div class="group-container">
                                <p class="group-label">Shop URL</p>
                                <a href="<%=gpu.getShop_URL()%>"><p class="group-content">Link</p></a>
                            </div>
                            <div class="group-container">
                                <p class="group-label">TDP</p>
                                <p class="group-content"><%=gpu.getTdp()%>W</p>
                            </div>
                            <div class="group-container">
                                <p class="group-label">Memory</p>
                                <p class="group-content"><%=gpu.getMemory()%></p>
                            </div>
                            <div class="group-container">
                                <p class="group-label">Memory Clock</p>
                                <p class="group-content"><%=gpu.getMemory_clock()%>MHz</p>
                            </div>
                            <div class="group-container">
                                <p class="group-label">Core Clock</p>
                                <p class="group-content"><%=gpu.getCore_clock()%>MHz</p>
                            </div>
                            <div class="group-container">
                                <p class="group-label">Boost Clock</p>
                                <p class="group-content"><%=gpu.getBoost_clock()%>MHz</p>
                            </div>
                            <div class="group-container">
                                <p class="group-label">Lenght</p>
                                <p class="group-content"><%=gpu.getLenght()%>mm</p>
                            </div>
                            <div class="group-container">
                                <p class="group-label">Slot Width</p>
                                <p class="group-content"><%=gpu.getSlot_width()%><%=gpu.getSlot_width() > 1 ? "Slots" : "Slot"%></p>
                            </div>
                            <div class="group-container">
                                <p class="group-label">Power Cable</p>
                                <p class="group-content"><%=gpu.getPower_cable()%></p>
                            </div>
                        </div>

                        <form class="component-buttons-container">
                            <input type="hidden" name="id" value="<%=gpu.getId()%>">
                            <button type="submit" formmethod="get" formaction="editGpu"><i class="fa fa-pencil" aria-hidden="true"></i>Edit</button>
                            <button type="button" class="remove-button" data-id="<%=gpu.getId()%>" data-filename="<%=fileName%>"><i class="fa fa-times-circle-o" aria-hidden="true"></i>Remove</button>
                        </form>
                    </div>
                <%}%>
            </div>
            ...
            <div id="remove-popup" class="popup">
                <div class="popup-content">
                    <h2></h2>
                    <form id="remove-form" action="removeGpu" method="post">
